...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    /* Exercicio 04 complexidade assintotica com limite superior O(h) onde h é a altura da árvore*/
    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax){
        // retorna falso se nao existem valores fora do intevalo
        if (min().compareTo(chaveMin) > 0 && max().compareTo(chaveMax) < 0 && raiz != null)
            return false;
        else{
            // enquanto houver valores fora do intervalo retira o maximo e o minimo
            while (min().compareTo(chaveMin) < 0 || max().compareTo(chaveMax) > 0){
                if (min().compareTo(chaveMin) < 0 )
                    deleteMin();
                if (max().compareTo(chaveMax) > 0)
                    deleteMax();
            }
            return true;
        }
