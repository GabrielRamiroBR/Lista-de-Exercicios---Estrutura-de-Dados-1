...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    /* complexidade O(n²)sendo n a quantidade de nós da árvore*/
    public void transformaArvBinBusca(Arvbin<Chave> arvoreBinaria){
        if (arvoreBinaria.retornaVal() == null)
            return;
        else{

           raiz = insere(raiz,new No (arvoreBinaria.retornaVal(), null));

           if (arvoreBinaria.retornaEsq() != null){
               transformaArvBinBusca(arvoreBinaria.retornaEsq());
           }
           if (arvoreBinaria.retornaDir() != null){
               transformaArvBinBusca(arvoreBinaria.retornaDir());
           }
        }
    }
    private No insere(No raiz, No x){
        if (raiz == null){
            return x;
        }
        if (raiz.chave.compareTo(x.chave) > 0){
            raiz.esq = insere(raiz.esq, x);
        }
        else if (raiz.chave.compareTo(x.chave) < 0){
            raiz.dir = insere(raiz.dir, x);
        }
        else
            raiz.valor = x.valor; // se a chave for igual atualiza o valor do elo
        return raiz;
    }
