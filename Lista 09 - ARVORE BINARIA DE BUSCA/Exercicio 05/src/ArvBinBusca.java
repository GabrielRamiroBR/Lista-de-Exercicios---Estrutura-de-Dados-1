...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
 ...
 /* Exercicio 05 - complexidade assintotica de limite superior O(h) sendo h a altura da árvore
 * pois a cada verificaçao se desce um nível*/
    public No obtemAncestralComum(No primeiroNo, No segundoNo){
        return obtemAncestralComum(raiz, primeiroNo, segundoNo);
    }
    public No obtemAncestralComum(No raiz, No primeiroNo, No segundoNo){
        if (raiz == null)
            return null;
            // se algum nó for igual a raiz o menor ancestral em comum é a propria raiz
        else if (raiz.chave.compareTo(primeiroNo.chave) == 0 || raiz.chave.compareTo(segundoNo.chave) == 0)
            return raiz;
            // se a chave de um nó for maior que o da raiz e a do outro nó for menor que a da raiz
            // o menor ancestral comum é a raiz
        else if ((primeiroNo.chave.compareTo(raiz.chave) < 0 && segundoNo.chave.compareTo(raiz.chave) > 0)
                || (primeiroNo.chave.compareTo(raiz.chave) > 0 && segundoNo.chave.compareTo(raiz.chave) < 0))
            return raiz;
        else{
            // chama recursivamente o metodo tendo a raiz como o nó a esquerda se ambas as chaves
            // forem menores que a da raiz corrente
            // e o nó a direita se ambas as chaves forem maiores que a da raiz corrente
            if (primeiroNo.chave.compareTo(raiz.chave) < 0 && segundoNo.chave.compareTo(segundoNo.chave) < 0)
                obtemAncestralComum(raiz.esq, primeiroNo, segundoNo);
            else
                obtemAncestralComum(raiz.dir, primeiroNo, segundoNo);
            return raiz;
        }
    }
