...
public class Arvbin<T extends Comparable<T>>
{
	...
	/*Exercício 03
	 * Complexidade O(n) sendo n a quantidade de nós da árvore corrente
	 * pois o método precisa ser evocado para cada nó da árvore no pior caso*/
	public boolean eSimilar(Arvbin<T> arvore) {
		if (val.compareTo(arvore.val) != 0) {
			return false;
		}

		else {
			boolean testeEsq = true, testeDir = true;
			if (esq != null && arvore.esq != null)
				testeEsq = esq.eSimilar(arvore.esq);
			else if ((esq == null && arvore.esq != null) || (esq != null && arvore.esq == null))
				return false;

			if (dir != null && arvore.dir != null)
				testeDir = dir.eSimilar(arvore.dir);
			else if ((dir == null && arvore.dir != null) || (dir != null && arvore.dir == null))
				return false;

			return testeEsq && testeDir;
		}
	}
