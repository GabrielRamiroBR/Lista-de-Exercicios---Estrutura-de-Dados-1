...
public class Arvbin<T extends Comparable<T>>
{
	...
	/*Exercício 02
	* Complexidade O(n) sendo n a quantidade de nós da árvore binária que entra como parametro no método
	* pois o método precisa ser evocado para cada nó da árvore*/
	public static Integer retornaSomaSubArvore(Arvbin<Integer> no){
		...
		if (no.val == null) {
			...
			if (no.esq != null){
				somaEsq = retornaSomaSubArvore(no.esq);
			}
			if (no.dir != null){
				somaDir = retornaSomaSubArvore(no.dir);
			}
